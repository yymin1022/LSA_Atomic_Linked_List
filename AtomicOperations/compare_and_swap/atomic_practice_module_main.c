...
static int	atomic_add_func(void *arg)
{
	...
	while (!kthread_should_stop())
	{
		...
		num = __sync_val_compare_and_swap(&counter, counter, counter + 1);		
...
int		__init atomic_practice_init(void)
{
	...
	printk ("Atomic with compare_and_swap Start: 20194094 Yongmin Yoo\n");
...
void		__exit atomic_practice_cleanup(void)
{
	...
	kfree(threads);
	printk ("Atomin with compare_and_swap Exit: Module Removed\n");
}
